...
import { createDiscussionReport } from '../api/discussionReport.api';
import { getAllCourses } from '../api/course.api';
import { MessageSquare, ThumbsUp, CheckCircle, Lock, Pin, Send, Plus, Filter, Trash2, Shield, Unlock, ArrowLeft, Flag, AlertTriangle, X } from 'lucide-react';
...
export default function Discussions() {
  ...
  const [reportTarget, setReportTarget] = useState(null); // { targetType: 'discussion' | 'reply', targetId: string, title?: string }
  const [reportReason, setReportReason] = useState('inappropriate_content');
  const [reportDescription, setReportDescription] = useState('');
  const [isSubmittingReport, setIsSubmittingReport] = useState(false);

  const openReportModal = (targetType, targetId, title = '') => {
    if (!user) {
  ...
  const handleReportSubmit = async (e) => {
    e.preventDefault();
    if (!reportTarget?.targetId || !reportReason) return;
    setIsSubmittingReport(true);
    try {
      await createDiscussionReport({
    ...
    <>
              ...
              <div className="border-b border-gray-100 dark:border-slate-800 pb-4 mb-4">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => openReportModal('discussion', current._id, current.title)}
                      className="flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1.5 rounded-xl border border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 text-gray-500 hover:text-amber-600 dark:text-gray-400 dark:hover:text-amber-400 hover:border-amber-200 transition cursor-pointer"
                      ...
                      >
                        <div className="flex justify-between items-start mb-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              onClick={() => openReportModal('reply', r._id, `Reply by ${r.user?.fullName || r.user?.name || 'User'}`)}
                              className="text-xs text-gray-400 hover:text-amber-600 p-1 transition"
      ...
      {reportModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 dark:border-slate-800 space-y-4">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2 text-rose-600">
                <AlertTriangle className="w-5 h-5" />
                <h3 className="text-base font-bold text-gray-900 dark:text-white">
                  Report {reportTarget?.targetType === 'discussion' ? 'Discussion' : 'Reply'}
                </h3>
              </div>
              <button
            ...
            <form onSubmit={handleReportSubmit} className="space-y-3.5 text-xs">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-rose-500 focus:outline-none"
                >
                  <option value="inappropriate_content">Inappropriate Content</option>
                  <option value="spam">Spam / Advertising</option>
                  <option value="harassment">Harassment / Bullying</option>
                  <option value="abusive_language">Abusive / Offensive Language</option>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full border border-gray-200 dark:border-slate-700 rounded-xl p-2.5 text-xs bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-rose-500 focus:outline-none placeholder:text-gray-400"
                  maxLength={1000}
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-slate-800">
                <button
                  type="button"
                  onClick={() => setReportModalOpen(false)}
                  className="px-3.5 py-2 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-slate-800 font-semibold"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-bold transition disabled:opacity-50 inline-flex items-center gap-1.5 shadow-sm"
                >
                  <Flag className="w-3.5 h-3.5" />
