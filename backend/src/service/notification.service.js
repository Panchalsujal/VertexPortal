...
import { sendNotificationEmail } from "./notificationEmail.service.js";
...
export async function createNotification({
  ...
  if (normalizedTitle.length > 200) {
    throw new ApiError(400, "Notification title cannot exceed 200 characters");
  }
  ...
  const parsedType =
    parseEnumQuery(type, NOTIFICATION_TYPES, "Notification type") ?? "system";
  ...
  const preferences = await NotificationPreference.findOne({
    ...
    .select(`inApp.${parsedType}`)
    .lean();

  const inAppEnabled = preferences?.inApp?.[parsedType] ?? true;
  ...
  if (expiresAt) {
    ...
    if (Number.isNaN(parsedExpiresAt.getTime())) {
      throw new ApiError(400, "Invalid notification expiry date");
    }
  }

  const normalizedActionUrl = String(actionUrl || "").trim();

  if (normalizedActionUrl.length > 2000) {
    throw new ApiError(
      400,
...
export async function dispatchNotification({
  ...
  try {
    inAppResult = await createNotification({
      userId,
      title,
      message,
      type,
    ...
    inAppResult = {
      ...
      reason: error?.message || "In-app notification failed",
    };
  }
...
export async function createBulkNotifications({
  ...
  const preferenceDocuments = await NotificationPreference.find({
    user: {
      $in: uniqueUserIds,
    },
  })
    .select(`user inApp.${parsedType}`)
    .lean();

  const preferenceMap = new Map(
    preferenceDocuments.map((preference) => [
      preference.user.toString(),

      preference.inApp?.[parsedType] ?? true,
    ]),
  );

  const enabledUserIds = uniqueUserIds.filter(
    (userId) => preferenceMap.get(userId) ?? true,
  );
  ...
  const documents = enabledUserIds.map((userId) => ({
    ...
    course: courseId ? new mongoose.Types.ObjectId(courseId) : null,
...
export async function getMyNotifications({ userId, query = {} }) {
  ...
  const parsedIsRead = parseBooleanQuery(isRead, "isRead");
  ...
  const {
    ...
    allowedFields: ["createdAt", "updatedAt", "readAt"],
  ...
  return {
    ...
    filters: {
      ...
      isRead: parsedIsRead ?? null,

      isArchived: parsedIsArchived ?? false,
...
export async function restoreNotification({ userId, notificationId }) {
  validateObjectId(userId, "user ID");
  ...
  if (!notification.isArchived) {
    return {
      ...
      message: "Notification is already active",
    };
  }
...
export async function updateNotificationPreferences({ userId, payload }) {
  ...
  for (const channel of allowedChannels) {
    ...
    if (
      ...
      throw new ApiError(400, `${channel} preferences must be an object`);
    }

    for (const [type, value] of Object.entries(payload[channel])) {
      ...
      const parsedValue =
        typeof value === "boolean"
          ? value
          : parseBooleanQuery(value, `${channel}.${type}`);

      update[`${channel}.${type}`] = parsedValue;
    }
  }
...
export async function dispatchBulkNotifications({
  ...
  try {
    inAppResult = await createBulkNotifications({
      userIds,
      title,
      message,
      type,
    ...
    console.error("Bulk in-app notification failed:", error);
  }
  ...
  for (const userId of userIds) {
    ...
    try {
      ...
      console.error(`Notification email failed for user ${userId}:`, error);
    }
  }
